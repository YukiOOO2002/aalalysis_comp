...
# group_type 列の作成（participant.labelの先頭を抽出）
df <- df %>%
  mutate(
...
# 【ここを修正しました】: contributeが0でないことを最大値フラグの条件に追加
calculate_max_flags <- function(data, round_num) {
  ...
  data <- data %>%
    ...
    mutate(
      ...
      # payoffの最大値と比較してフラグを設定（こちらは条件変更なし）
...
plot_max_c_and_p_by_round <- df_summary %>%
  ...
  labs(
    title = "the proportion of simultanious gain of max contribute and payoff",
    x = "round",
    y = "proportion",
    color = "treatment"
  ) +
...
all_trend <- paste0(save_dir, "max_cont_and_payoff_proportion except c=0.png")
ggsave(
  filename = all_trend,  
